...
class Complex
{
    ...
    public:
        ...
        Complex operator+(Complex);			// Binary operator is member function here so it needs only one argument.
        void display(void);
};
...
{
    Complex temp;
    temp.x = x + c.x;           			// x belongs to C1 and c.x belongs to C2.
    temp.y = y + c.y;           			// y belongs to C1 and c.y belongs to C2.
